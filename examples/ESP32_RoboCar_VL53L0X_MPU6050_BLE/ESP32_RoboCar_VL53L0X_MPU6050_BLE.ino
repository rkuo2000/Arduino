...
float  Heading, HeadingTgt;
int angle;

// IR Ranger control/status vars
#define RangerOffset 30 //mm
int distance;
...
void loop() {
  ...
  if ((millis() - lastTime)> loopDelay){
    ...
    distance = ranger.readRangeContinuousMillimeters() - RangerOffset; // from IR ranger
    Heading  = GetHeading(); // from IMU
    
    txString = "D=" + String(distance) +" H=" +String(Heading);
