...
AfstandsSensor afstandssensor(trigPin,echoPin); // GPIO13 to trigPin, GPIO12 to echoPin
...
void loop() {
  ...
  if (millis()-lastTime >= loopTime) { 
    lastTime = millis(); 
    distance = afstandssensor.afstandCM(); // read Distance from SR04 
    readIMU(); //read IMU to get YPR  
    
    tx = String(distance)+","+String(YPR.x)+","+String(YPR.y)+","+String(YPR.z); 
    Serial.println(tx);
    BLEprint(tx); // BLE send tx string
    lastYPR = YPR; 
  }  
  ...
  // check BLE command
  if (BLEflag) {
    Serial.print(BLEcmd); Serial.print(" "); Serial.println(BLErxbuf);
