...
#define DEFAULTSPEED 1023 // ModeMCU = 1023,UNO = 255  
#define DEFAULTFREQ  1000 // 1KHz
...
class Motor
{
  public:
    ...
    Motor(int In1pin, int In2pin, int PWMpin, int PWMch, int offset, int STBYpin);      

    // Drive in direction given by sign, at speed given by magnitude of the 
	//parameter.
    void drive(int speed, int ch);  
  ...
  private:
	...
	int In1, In2, PWM, channel, Offset, Standby;
	
	//private functions that spin the motor CC and CCW
	void fwd(int speed, int ch);
	void rev(int speed, int ch);
