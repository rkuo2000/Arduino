...
// Bluetooth connected to PS4 controller
...
void loop() {
  ...
  // read PS4 controller to set motor driver
  if (PS4.isConnected()) {

    if (PS4.Up())    { forward(motorR, motorL, Speed); Dir=true;}    
    if (PS4.Down())  { backward(motorR, motorL, Speed); Dir=false;}
    if (PS4.Left())  { left(motorR, motorL, Speed); Dir=false;}
    if (PS4.Right()) { right(motorR, motorL, Speed); Dir=false;}    
    if (PS4.Cross()) { brake(motorR, motorL); Dir=false;}
  }

  if (Dir && (distance < SafeDistance)) brake(motorR, motorL);  

  delay(20); // delay 20ms for next motor driving
