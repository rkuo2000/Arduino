...
// using PCA9685 to drive SG90 x12
...
void handleWebSocketMessage(void *arg, uint8_t *data, size_t len) {
  ...
  if (info->final && info->index == 0 && info->len == len && info->opcode == WS_TEXT) {
    ...
    delay(1000); // keep PWM running for a second
    // set PWM zero pulse to save power
    pca9685.setPWM(servoPin,   0, 0); // set PWM output Low
    pca9685.setPWM(servoPin+1, 0, 0); // set PWM output Low
    pca9685.setPWM(servoPin+2, 0, 0); // set PWM output Low        
