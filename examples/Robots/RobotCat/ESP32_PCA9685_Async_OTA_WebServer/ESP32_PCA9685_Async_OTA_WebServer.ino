//
// ESP32 Async OTA WebServer
//
// PCA9685 for 12ch PWMs output SG90 x12 (+5V, Vcc, SCL, SDA, Gnd)
...
<body>
  <div class="topdiv">
    <h1>ESP32 Async OTA WebServer</h1> 
    <h2>PCA9685 driving SG90 x12</h2>
  </div>
  <div class="content">
    <div>
      <table>  
...
void handleWebSocketMessage(void *arg, uint8_t *data, size_t len) {
  ...
  if (info->final && info->index == 0 && info->len == len && info->opcode == WS_TEXT) {
    ...
    delay(30); // keep PWM running for a second
    // set PWM zero pulse to save power
    setServoOff(servoPin);
    setServoOff(servoPin+1);
    setServoOff(servoPin+2); 
  }
}
...
void setServoOff(int pinSERVO){
  pca9685.setPWM(pinSERVO, 0, 4096);
  delay(30);
}
