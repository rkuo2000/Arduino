...
// DRV8833
// nSLEEP (EEP) connected to Vcc
// nFALULT kept unconnected (floating)
#define Motor_0A 5     // IN1 to control MotorA
#define Motor_0B 6     // IN2 to control MotorA
#define Motor_1A 7     // IN3 to control MotorB
#define Motor_1B 8     // IN4 to control MotorB

// For Six-Legged Robot
void forward()
{
    digitalWrite(Motor_0A, 1);
    digitalWrite(Motor_0B, 0);
    delay(50);
}

void backward()
{
    digitalWrite(Motor_0A, 0);
    digitalWrite(Motor_0B, 1); 
    delay(50);
}
...
void setup()
{
    ...
    // set Motor pins & default
    pinMode(Motor_0A, OUTPUT);
    pinMode(Motor_0B, OUTPUT);
...
{  
    ...
    if (turncount<6 && skipcount>OFFSET) {
        turnleft(); 
        delay(DELAY_1_ROUND/6); // 假設1200ms 轉一圈
        stop();
        ask_VLM();
        turncount++;
    }
